...
>**谨记YAGIN原则，不要写程序目前用不上，但是将来可能会使用的功能代码，You're NOT Goona Need It!**  

srds  待实现功能
1. 调试信息太多未清理
2. 证书超时检测通过修改系统时间调整(不会别的方法)
3. ssl认证部分随便写的，能跑就没仔细管
4. autogenerate脚本临时写的，可能有错误风险，跑完了检查是否有空文件，有的话就失败了
5. 希望客户端增加信号处理函数，kill完事后，调用close优雅关闭连接  
6. ~~如果将来闲的蛋疼的时候，兴许再写个压力测试~~
